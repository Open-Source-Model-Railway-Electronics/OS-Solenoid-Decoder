...
It can handle loads of up to 5 amps and supports almost any type of point motor. It can also be used in combination with OS-relays to handle frog polarisation for electrofrog and unifrog points.
...
Note, the General Purpose relay extension can also be used to power electrofrog points, only with a little bit more wiring. You need to use to the NO contacts, and loop both COM contacts. The COM contacts can be connected to the frog. The NO contacts can be connected to the track power. 
...
One LED ON + other LED  blink	Electro frog Mode (double pulse + frog)
...
In Electrofrog mode, outputs 1–4 control the relays on outputs 8–5 in reversed 
order (1→8, 2→7, etc.).
...
996 or 1000	Disable Roco 4-address offset (default)
997 or 1001	Enable Roco 4-address offset
